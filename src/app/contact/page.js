...
export default function ContactPage() {
  ...
  useEffect(() => {
    const loadDistrict = async () => {
      ...
      try {
        const snap = await getDoc(
          doc(db, "websites", "spinreactin", "districts", currentDistrict)
        );
        if (snap.exists()) {
                ...
                {loading ? (
                  <div className="space-y-4">
                    {[...Array(3)].map((_, i) => (
                      <div
                        key={i}
                        className="h-28 rounded-3xl bg-white animate-pulse border border-slate-200"
                      />
                    ))}
                  </div>
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-white p-8 text-center text-slate-500 shadow-sm">
                    ...
                    <p className="font-semibold text-[#0F172A]">No Contact Information Added</p>
                  ...
                  contactInfo.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-start gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-md transition-all hover:border-amber-500/50 hover:shadow-xl"
                      >
                        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-amber-500 shrink-0 border border-slate-200">
                          {getFieldIcon(label)}
                        </div>
                              ...
                              if (isPhone) {
                                return (
                                  <a
                                    ...
                                    className="text-base sm:text-lg font-bold text-[#0F172A] hover:text-amber-600 transition-colors inline-block"
                                  >
                                    {val}
                                  </a>
                                );
                              ...
                              if (isEmail) {
                                return (
                                  <a
                                    key={vIdx}
                                    href={`mailto:${val}`}
                                    className="text-base font-bold text-[#0F172A] hover:text-amber-600 transition-colors inline-block break-all"
                                  >
                                    {val}
                                  </a>
                                );
          ...
          <div className="mt-12 space-y-4">
            {faqs.map((faq, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-3xl border border-slate-200 bg-white overflow-hidden transition-all shadow-md"
                >
                  <button
                    type="button"
                    onClick={() => setOpenFaq(isOpen ? null : idx)}
                    className="w-full flex items-center justify-between p-6 text-left font-bold text-[#0F172A] hover:text-amber-600 cursor-pointer"
                  >
                  ...
                  {isOpen && (
                    <div className="px-6 pb-6 text-sm sm:text-base leading-relaxed text-slate-600 border-t border-slate-100 pt-4">
                      {faq.a}
                    </div>
                  )}
                </div>
